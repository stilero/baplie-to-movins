...
require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_convertbapliemovins'.DS.'helpers'.DS.'query.php');

class ConvertbapliemovinsHelper
{
	...
	public static function addSubmenu($vName = 'settings')
	{
		...
		JSubMenuHelper::addEntry(
			JText::_('Settings'),
			'index.php?option=com_convertbapliemovins&view=settings',
			($vName == 'settings')
		);
	...
	public static function getExtensions() 
	{
		...
		$jv = new JVersion();
		$alt_libdir = ($jv->RELEASE < 1.6) ? JPATH_ADMINISTRATOR.DS.'components'.DS.'com_convertbapliemovins' : null;
		JLoader::import('joomla.utilities.xmlelement', $alt_libdir);
		
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_convertbapliemovins'.DS.'elements'.DS.'extensions.xml', 'JXMLElement');		        
 ...
 * @since		1.5
 */
abstract class JHtmlConvertbapliemovins
{
	/**
	...
	public static function extensions($ext) 
	{
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_convertbapliemovins.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__convertbapliemovins_categories AS a');
