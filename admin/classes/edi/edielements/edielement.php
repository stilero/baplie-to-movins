...
abstract class EdiElement {
    ...
    protected function arrayToString(){
        $string = '';
        $segmentArray = $this->getArray();
        //Convert sub arrays to strings
        for( $i=0 ; $i < count($segmentArray) ; $i++ ){
            if(is_array($segmentArray[$i])){
                for( $j=count( $segmentArray[$i])-1 ; $j >= 0 ; $j-- ){
                    if( is_null( $segmentArray[$i][$j] ) ){
                        unset($segmentArray[$i][$j]);
                    }
                }
                $segmentArray[$i] = implode(':', $segmentArray[$i]);
            }
        }
        $string = implode('+', $segmentArray);
        return $string;
    }
